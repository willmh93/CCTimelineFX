    ...
    bool Particle::Update()
    {
        ...
        if (_age > _lifeTime || _dead == 2)                 // if dead=2 then that means its reached the end of the line (in kill mode) for line traversal effects
        {
            ...
            if (_children.empty())
            {
				...
				//assert(_emitter->IsGroupParticles() == this->IsGroupParticles());

                if (IsGroupParticles()) // CCTimelineFX Bug fix: Crash if Emitter::IsGroupParticles() is true when Particle::IsGroupParticles() is false (incompatible p->_listIter)
