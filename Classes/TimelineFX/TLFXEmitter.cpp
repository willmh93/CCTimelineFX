...
//     }
    ...
    bool Emitter::Update()
    {
        ...
        _matrix.Set(cosf(_angle / 180.0f * (float)PI), sinf(_angle / 180.0f * (float)PI), -sinf(_angle / 180.0f * (float)PI), cosf(_angle / 180.0f * (float)PI));
            ...
            for (int c = 1; c <= intCounter; ++c)
            {
                ...
                if (e)
                {
                    ...
                    case Effect::TypeEllipse:
                        {
							...
							// Fix to prevent TransformVector(...) on a null-vector. Ensure size is never 0.
							if (tx < 0.0001f)
								tx = 0.0001f;
							if (ty < 0.0001f)
								ty = 0.0001f;
                            ...
                            e->SetX( cosf(th / 180.0f * (float)PI) * tx - _parentEffect->GetHandleX() + tx);
                            e->SetY(-sinf(th / 180.0f * (float)PI) * ty - _parentEffect->GetHandleY() + ty);
                    ...
                    if (_lockedAngle)
                    {
                        ...
                        {
                            e->SetSpeedVecX(sinf(e->GetEntityDirection() / 180.0f * (float)PI));
                            e->SetSpeedVecY(cosf(e->GetEntityDirection() / 180.0f * (float)PI));
                    ...
                    {  // @todo dan Set(cosf(_angle  ??
                        float angle = _angle / 180.0f * (float)PI;
