...
namespace TLFX
{
    ...
    {
		auto it = std::find(_unused.begin(), _unused.end(), p);

		// CCTimelineFX: Nasty hack, ReleaseParticle would release
		// the same particle twice when owned by both Effect and Child
		// Emitter (Bonus Bursts/Coin Stacks 3). Converted stack to
		// vector to allow for random access and only remove 'p' once.
		if (it == _unused.end())
		{
			--_inUseCount;
			_unused.push_back(p);
    ...
    void ParticleManager::DrawParticles( float tween /*= 1.0f*/, int layer /*= -1*/ )
    {
        ...
        */
        ...
        if (_angle != 0)
        {
            ...
            _matrix.Set(cosf(_angleTweened / 180.0f * (float)PI), sinf(_angleTweened / 180.0f * (float)PI), -sinf(_angleTweened / 180.0f * (float)PI), cosf(_angleTweened / 180.0f * (float)PI));
        }
    ...
    void ParticleManager::SetOrigin( float x, float y, float z /*= 1.0f*/ )
    {
        ...
        _oldOriginZ = _originZ;
        _originX = x;
        _originY = -y; // CCTimelineFX - Modified to reflect flipped coordinate system
        _originZ = z;
    }
	...
	std::set<Effect*>::iterator ParticleManager::RemoveEffect(Effect* e)
	{
		auto &el = _effects[e->GetEffectLayer()];

		std::set<Effect*>::iterator it = std::find(el.begin(), el.end(), e);
		std::set<Effect*>::iterator nextIt = el.end();
        ...
        if (p->GetAge() != 0 || p->GetEmitter()->IsSingleParticle())
        {
            ...
            if (_px > _vpX - p->GetImageDiameter() && _px < _vpX + _vpW + p->GetImageDiameter() && _py > _vpY - p->GetImageDiameter() && _py < _vpY + _vpH + p->GetImageDiameter())
            {
                if (p->GetAvatar())
                {
					...
					float fw = p->GetAvatar()->GetWidth();
					float fh = p->GetAvatar()->GetHeight();
                    DrawSprite(sprite, _px, _py, _tv, fw, fh, x, y, rotation, scaleX, scaleY, r, g, b, a, blend == Emitter::BMLightBlend);
					// ++rendercount
                }
            }
        }
