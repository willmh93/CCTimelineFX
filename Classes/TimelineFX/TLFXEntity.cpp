...
namespace TLFX
{
    ...
    bool Entity::Update()
    {
        ...
        if (_updateSpeed && _speed)
        {
            ...
            _speedVec.x = sinf(_direction / 180.0f * (float)PI) * _pixelsPerSecond;
            _speedVec.y = cosf(_direction / 180.0f * (float)PI) * _pixelsPerSecond;
        ...
        if (_relative)
            _matrix.Set(cosf(_angle / 180.f * (float)PI), sinf(_angle / 180.0f * (float)PI), -sinf(_angle / 180.0f * (float)PI), cosf(_angle / 180.0f * (float)PI));
    ...
    {
        _matrix.Set(cosf(_angle / 180.0f * (float)PI), sinf(_angle / 180.f * (float)PI), -sinf(_angle / 180.0f * (float)PI), cosf(_angle / 180.0f * (float)PI));
